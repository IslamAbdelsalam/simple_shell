#include "shell.h"

/**
 * from_terminal - check we in interactive mode? or not
 * @info: struct address
 *
 * Return: 1 if from_terminal mode else 0
 */

int from_terminal(NodeInfo *info)
{
	return (isatty(STDIN_FILENO) && info->fd_read <= 2);
}

/**
 * _isalpha - check is it alphabetic characters or not
 * @c: The character to input
 * Return: 1 if c is alphabetic else 0 if not
 */

int _isalpha(int c)
{
	if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
		return (1);
	else
		return (0);
}

/**
 * _atoi - converts a string to an integer
 * @s: the string to be converted
 * Return: 0 if no numbers in string else converted integer
 */

int _atoi(char *s)
{
	int i, sign = 1, flag = 0, output;
	unsigned int result = 0;

	for (i = 0; s[i] != '\0' && flag != 2; i++)
	{
		if (s[i] == '-')
			sign *= -1;
		if (s[i] >= '0' && s[i] <= '9')
		{
			flag = 1;
			result *= 10;
			result += (s[i] - '0');
		}
		else if (flag == 1)
			flag = 2;
	}
	if (sign == -1)
		output = -result;
	else
		output = result;
	return (output);
}

/**
 * delimiterChecker - checks the delimiter char
 * @c: Character to be checked.
 * @delimiters: String of charaters to be compated.
 *
 * Return: True if it is a delimiter, else false.
 */

bool delimiterChecker(char c, char *delimiters)
{
	int s;

	for (s = 0; delimiters[s]; s++)
		if (c == delimiters[s])
			return (true);
	return (false);
}
